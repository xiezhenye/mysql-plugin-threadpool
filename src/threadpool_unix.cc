...
{
  //XZY
  const ulong mode= TP_HIGH_PRIO_MODE_STATEMENTS;//c->thd->variables.threadpool_high_prio_mode;
  ...
  for(;;)
  {
    ...
    */
    for(int i=(listener_picks_event)?1:0; i < cnt ; i++)
    {
      ...
      {
        //XZY
        c->tickets= UINT_MAX; //c->thd->variables.threadpool_high_prio_tickets;
...
static void queue_put(thread_group_t *thread_group, connection_t *connection)
{
  ...
  mysql_mutex_lock(&thread_group->mutex);
  //XZY
  connection->tickets= UINT_MAX; //connection->thd->variables.threadpool_high_prio_tickets;
